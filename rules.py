...
def get_external_id_values(element, value_type, return_elements=False):
  """Helper to gather all Values of external ids."""
  external_ids = element.findall(".//ExternalIdentifier")
  values = []
  for extern_id in external_ids:
    id_type = extern_id.find("Type")
    if id_type is not None and id_type.text:
      check_value = False
      id_text = id_type.text.strip()
      if id_text == "other":
        other_type = extern_id.find("OtherType")
        if other_type is not None and other_type.text.strip() == value_type:
          check_value = True
      elif id_text == value_type:
        check_value = True
      if check_value:
...
class ValidStableID(base.BaseRule):
  ...
  def check(self, element):
    ...
    error_log = [
        base.ErrorLogEntry(
            None, "Stable id {} is not in the correct format.".format(s_id))
        for s_id in stable_ids if not self.stable_id_matcher.match(s_id)
    ]
    if error_log:
      raise base.ElectionError(
      ...
      if not external_ids:
        error_log.append(
            base.ErrorLogEntry(
                ...
                "contest %s does not have any external identifiers" %
...
class OfficesHaveJurisdictionID(base.BaseRule):
  ...
  def check(self, element):
    ...
    jurisdiction_values.extend(
        list(
            filter(str.strip, get_external_id_values(element,
                                                     "jurisdiction-id"))))
